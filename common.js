...
/* ═══════════════════════════════════════════════════════════════════
   VALYUTA KURSI — endi REAL API orqali (fawazahmed0/currency-api —
   bepul, kalit kerak emas, cheklovsiz, jsDelivr CDN). Agar tarmoq
   muammosi bo'lsa (CDN ishlamasa), avtomatik ravishda admin qo'lda
   kiritgan/standart qiymatga qaytadi — sahifa hech qachon buzilmaydi.
   Freshness: 6 soatdan kam bo'lsa qayta so'rov yubormaymiz (keraksiz
   tarmoq yukini kamaytirish uchun).
   ═══════════════════════════════════════════════════════════════════ */
TimartShared.getExchangeRate = function() {
  try {
    const saved = localStorage.getItem("timart_exchange_rate");
    return saved ? JSON.parse(saved) : { krwToUzs: 7.97, updatedAt: "2026-06-22", source: "standart" };
  } catch (e) { console.warn("timart_exchange_rate buzilgan", e); return { krwToUzs: 7.97, updatedAt: "2026-06-22", source: "standart" }; }
};

TimartShared.setExchangeRate = function(rate, date, source) {
  localStorage.setItem("timart_exchange_rate", JSON.stringify({ krwToUzs: rate, updatedAt: date, source: source || "admin" }));
};
...
TimartShared.refreshExchangeRateFromAPI = async function() {
  ...
  try {
    ...
    const rate = data.krw && data.krw.uzs;
    if (!rate || rate <= 0) throw new Error("UZS kursi topilmadi");
    this.setExchangeRate(rate, new Date().toISOString().slice(0,10), "api");
